  ...
  const { id } = router.query; // Accessing the dynamic route parameter
        ...
        useEffect(() => {
            ...
            const fetchData = async () => {
                const response = await fetch(`http://localhost:3000/article/id/${id}`, {
                    method: "GET",
                ...
                if (response.ok) {
                    ...
                    setFormData(data); // Assuming the API response matches the formData structure
                } else {
            ...
            fetchData();
        }, []); // The dependency array is empty, meaning this effect runs once on mount
